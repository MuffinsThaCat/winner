#!/usr/bin/env python3
"""
Apples-to-Apples Comparison: Williams vs SupraBTM
Extracts overlapping blocks and compares performance
"""

import sys

def parse_williams_line(line):
    """Parse Williams output line: Block_No Threads Block_Size Williams_Time"""
...
def parse_supra_line(line):
    """Parse SupraBTM output line: Block_No Threads Block_Size Seq_Time iBTM_Time"""
    parts = line.strip().split('\t')
    if len(parts) >= 5 and parts[0].isdigit():
        ...
        return {
            ...
            'threads': int(parts[1]),
            'txs': int(parts[2]),
            'seq_time_ms': float(parts[3].replace('ms', '').replace('µs', '').replace('ns', '')),
            'ibtm_time_ms': time_ms
        }
    return None

print("Loading datasets...")

# Load SupraBTM blocks
supra_blocks = {}
...
# Load Williams blocks (streaming to avoid memory issues)
williams_blocks = {}
with open('results/williams_500_blocks.txt', 'r') as f:
    for line in f:
        data = parse_williams_line(line)
        if data:
            # Only keep blocks that overlap with SupraBTM
            if data['block'] in supra_blocks:
                williams_blocks[data['block']] = data

print(f"  Williams: {len(williams_blocks)} overlapping blocks")

# Find common blocks
common_blocks = sorted(set(supra_blocks.keys()) & set(williams_blocks.keys()))

print(f"\nCommon blocks: {len(common_blocks)}")

if len(common_blocks) == 0:
    print("\n❌ NO OVERLAPPING BLOCKS FOUND!")
    print(f"   Williams block range: Check your data")
    print(f"   SupraBTM block range: {min(supra_blocks.keys())}-{max(supra_blocks.keys())}")
    print("\n⚠️  You need to run Williams on the SAME blocks as SupraBTM!")
    print("\nSupraBTM tested these block ranges:")
    
    # Find contiguous ranges in SupraBTM
    supra_sorted = sorted(supra_blocks.keys())
    ranges = []
    start = supra_sorted[0]
    end = supra_sorted[0]
    
    for block in supra_sorted[1:]:
        if block == end + 1:
            end = block
        else:
    ...
    for start, end, count in ranges:
        print(f"   {start}-{end} ({count} blocks)")
...
supra_total_time = sum(supra_blocks[b]['ibtm_time_ms'] for b in common_blocks)
